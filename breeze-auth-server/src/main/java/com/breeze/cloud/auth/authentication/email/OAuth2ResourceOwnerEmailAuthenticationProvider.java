...
/**
 * oauth2资源所有者邮箱身份验证提供者
 *
 * @author gaoweixuan
 * @date 2023-04-21
 */
@Slf4j
public class OAuth2ResourceOwnerEmailAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        OAuth2ResourceOwnerEmailAuthenticationToken resourceOwnerEmailAuthenticationToken = (OAuth2ResourceOwnerEmailAuthenticationToken) authentication;

        OAuth2ClientAuthenticationToken clientAuthenticationToken = getAuthenticatedClientElseThrowInvalidClient(resourceOwnerEmailAuthenticationToken);
        ...
        if (!registeredClient.getAuthorizationGrantTypes().contains(CustomAuthorizationGrantType.EMAIL_CODE)) {
            throw new OAuth2AuthenticationException(OAuth2ErrorCodes.UNAUTHORIZED_CLIENT);
        }

        Authentication emailAuthentication = getEmailAuthentication(resourceOwnerEmailAuthenticationToken);

        // Default to configured scopes
        Set<String> authorizedScopes = registeredClient.getScopes();
        Set<String> requestedScopes = resourceOwnerEmailAuthenticationToken.getScopes();
        ...
        DefaultOAuth2TokenContext.Builder tokenContextBuilder = DefaultOAuth2TokenContext.builder()
                ...
                .authorizationGrantType(CustomAuthorizationGrantType.EMAIL_CODE)
                .authorizationGrant(resourceOwnerEmailAuthenticationToken);
        // @formatter:on
        ...
        OAuth2Authorization.Builder authorizationBuilder = OAuth2Authorization.withRegisteredClient(registeredClient)
                .principalName(emailAuthentication.getName())
                .authorizationGrantType(CustomAuthorizationGrantType.EMAIL_CODE)
                .authorizedScopes(authorizedScopes)
     ...
     * 解析用户名邮箱认证
     *
     * @param resourceOwnerEmailAuthenticationToken 资源所有者邮箱身份验证
     * @return {@link Authentication}
     */
    private Authentication getEmailAuthentication(OAuth2ResourceOwnerEmailAuthenticationToken resourceOwnerEmailAuthenticationToken) {

        Map<String, Object> additionalParameters = resourceOwnerEmailAuthenticationToken.getAdditionalParameters();

        String username = (String) additionalParameters.get(CustomOAuth2ParameterNames.EMAIL);
        String code = (String) additionalParameters.get(CustomOAuth2ParameterNames.CODE);

        EmailAuthenticationToken emailAuthenticationToken = new EmailAuthenticationToken(username, code);
